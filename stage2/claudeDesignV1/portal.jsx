...
const IDENTITY = {
  home:       { color: C.accent,    tint: C.accentTint },
...
const MOCK_TREE = [
  { id: 'my', name: 'My Dashboards', type: 'folder', count: 8, owner: 'You',
    children: [
      { id: 'my-q4', name: 'Q4 Review', type: 'dashboard' },
      { id: 'my-pipeline', name: 'Pipeline Health', type: 'dashboard' },
  ...
  { id: 'shared', name: 'Shared with me', type: 'folder', count: 12, owner: 'Team',
    children: [
      { id: 'sh-sales', name: 'Sales Performance', type: 'dashboard' },
  ...
  { id: 'donut_flat', name: 'donut_flat', type: 'dashboard' },
...
// ============================================================
const RailButton = ({ icon, label, active, onClick, identityKey = 'home' }) => {
  const id = IDENTITY[identityKey] || IDENTITY.home;
  return (
  <button
    onClick={onClick}
    title={label}
...
const LeftRail = ({ view, setView, isEmpty, showData, showHome }) => (
  <nav style={{
    ...
    {showHome && <RailButton icon="workspace" label="Workspace" active={view === 'home'} onClick={() => setView('home')} identityKey="home" />}
...
// ============================================================
// Top header — global search + create menu + workspace pill
// ============================================================
...
const WorkspaceMenu = ({ onSwitch, currentMode }) => {
  const items = [
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '6px 10px 4px', fontSize: 11, fontWeight: 600,
        color: C.inkSubtle, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Switch workspace</div>
      {items.map(it => {
...
const TopHeader = ({ tweaks, onToggleSidebar, onCreate, onSwitchWorkspace, onOpenAI, onOpenSecurityHome }) => {
    ...
    <header style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <button onClick={onToggleSidebar} style={iconBtn} title="Toggle tree">
          <Icon name="panel-left" size={18} color={C.inkMuted} />
        </button>
        <span style={{ fontWeight: 700, fontSize: 15, color: C.ink, letterSpacing: '-0.01em' }}>StyleBI</span>
        {tweaks.isSaaS && (
          <button style={{
            ...
            <Icon name="chevron-down" size={12} color={C.inkMuted} />
          </button>
        )}
        {tweaks.hasEMAccess && (
          <div ref={wsRef} style={{ position: 'relative' }}>
            ...
            onMouseLeave={(e) => {
              ...
              Dashboard Portal
              <Icon name="chevron-down" size={13} color={C.accentInk} />
            </button>
            {wsOpen && (
              <WorkspaceMenu currentMode="portal"
                onSwitch={(id) => { setWsOpen(false); onSwitchWorkspace(id); }} />
            )}
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <button
          ...
          title="AI Assistant">
          <Icon name="ai" size={15} color={C.inkMuted} />
          Ask AI
        </button>
        {tweaks.isDesigner && (
...
// ============================================================
const TreeRow = ({ node, depth = 0, expanded, onToggle, selected, onSelect }) => {
  ...
  return (
    <>
      ...
      >
        ...
        {node.starred && <Icon name="star-fill" size={12} color={C.accent} />}
      </div>
    ...
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '80px 40px 60px', textAlign: 'center' }}>
        ...
        <p style={{ margin: '10px 0 24px 0', fontSize: 14, color: C.inkMuted, lineHeight: 1.55, maxWidth: 460, marginInline: 'auto' }}>
          Once a designer on your team publishes dashboards to this workspace, you’ll be able to open them from here.
        </p>
        {aiVisible && (
...
const App = () => {
  ...
  const [view, setView] = useState(rawTweaks.securityEnabled && rawTweaks.role === 'viewer' ? 'dashboards' : 'home');
  // Keep view in sync with role: viewers can't sit on Workspace.
  useEffect(() => {
    if (!isDesigner && view === 'home') setView('dashboards');
  }, [isDesigner, view]);
  const [activeDashboard, setActiveDashboard] = useState(null);
  const [selectedTreeId, setSelectedTreeId] = useState(null);
  const [emGate, setEmGate] = useState(null); // null | 'security' | 'home'
  ...
  const railView = view === 'dashboard' ? 'home' : view;
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <TopHeader
          ...
          onSwitchWorkspace={(id) => {
            if (id === 'em') setEmGate('home');
          }}
          onOpenAI={() => setAiOpen(true)}
          onOpenSecurityHome={() => setView('home')}
        />
