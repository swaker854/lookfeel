...
function TweaksPanel({ title = 'Tweaks', noDeckControls = false, children }) {
  ...
  React.useEffect(() => {
    ...
    // Standalone fallback: if no host (e.g. GitHub Pages) sends
    // __activate_edit_mode within 400ms, open the panel ourselves so
    // viewers of the published page can still access Tweaks.
    const hostless = (window.parent === window) ||
      (typeof window.__OM_HOST === 'undefined' && !document.referrer.startsWith(location.origin));
    const t = setTimeout(() => { if (hostless) setOpen(true); }, 400);
